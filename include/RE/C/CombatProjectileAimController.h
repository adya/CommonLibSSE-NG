...
namespace RE
{
	...
	public:
		...
		// add
		void CalculateAim(Actor* target);

		// members
		BGSProjectile* projectile;                 // 48
		std::uint32_t  unk50;                      // 50
		std::uint32_t  groundRadius;               // 54
		float          turningAngle;               // 58
		NiPoint3       attackerLocation;           // 5C
		NiPoint3       projectileLaunchPoint;      // 68
		NiPoint3       aimPoint;                   // 74
		NiPoint3       targetBodyPartOffset;       // 80
		NiPoint3       aimOffset;                  // 8C
		...
		float          unkB0;                      // B0
		NiPoint3       lastKnownProjectileOffset;  // B4
